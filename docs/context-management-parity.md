...
The local backend mirrors the public **task/session and agent addressing contract** of Codex history/notes. It is not a replacement for OpenAI's encrypted model-only state. Native `openai-codex` continues to use the remote backend; provider selection and remote authentication/wire behavior are unchanged.
...
| OpenAI encrypted arguments/results and private reasoning state | Not reproduced or decrypted locally |
